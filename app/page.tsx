  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100 text-gray-800">
      ...
      <header className="bg-white shadow-md px-6 py-4">
        {/* Top row: Logo and mobile logout */}
        <div className="flex justify-between items-center w-full">
          ...
          {!user ? (
            <div className="flex items-center gap-2 md:gap-4">
              {" "}
              {/* Different gap on desktop */}
              <a
                href="/login"
                className="bg-white border border-blue-600 text-blue-600 px-4 py-1 rounded-full text-sm hover:bg-blue-50 md:px-6 md:py-2"
              >
                Login
              </a>
              <a
                href="/signup"
                className="bg-blue-600 text-white px-4 py-1 rounded-full text-sm hover:bg-blue-700 md:px-6 md:py-2"
              >
                Signup
              </a>
            </div>
        ...
        {/* Second row: Search Bar (mobile and desktop) */}
        {user && (
          <div className="mt-4 md:mt-0 md:absolute md:left-1/2 md:transform md:-translate-x-1/2 md:top-4 md:w-1/3">
                ...
                onChange={async (e) => {
                  ...
                  if (value.length > 1) {
                    const res = await fetch(
                      `/api/users/search?q=${value}&viewer_id=${user.id}`
                    );
                    const data = await res.json();
        ...
        {/* Third row: Mobile Navigation Buttons */}
        {user && (
          <div className="mt-4 flex md:hidden items-center gap-3 justify-center">
            <button
        ...
        {/* Desktop Navigation - positioned absolutely */}
        ...
        <div className="w-full max-w-xl bg-white p-6 rounded-2xl shadow-lg space-y-6">
            ...
            <>
              <div className="flex items-center space-x-4">
                <Image
                  src={
                    user?.avatarUrl ||
                    avatarUrlFor(user?.username || "User", 48)
                  }
                  alt="avatar"
                  width={48}
                  height={48}
          ...
          {posts.length === 0 ? (
            ...
            posts.map((post) => (
              <div
                key={post.id}
                className="border rounded-lg p-4 shadow-sm relative space-y-3"
              >
                <div className="flex items-center justify-between">
                  ...
                  {user && user.id !== post.user_id && (
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() =>
                          handleFollowToggle(post.user_id, post.is_following)
                        }
                        className={`text-sm px-3 py-1 rounded-full ${
                          post.is_following
                  ...
                  <div className="mt-3 bg-gray-50 rounded-lg p-3 space-y-2">
                    ...
                    {user && (
                      <div className="flex items-center gap-2 pt-2 border-t">
                        <Image
                          src={
                            user.avatarUrl || avatarUrlFor(user.username, 24)
                          }
                          alt="Your avatar"
                          width={24}
                          height={24}
